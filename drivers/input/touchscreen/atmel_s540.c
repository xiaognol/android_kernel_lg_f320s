...
#define MXT_LATEST_CONFIG_CRC	0x4A6626
u8 latest_firmware[] = {
...
#define MXT_POWER_OFF		0
#define MXT_POWER_ON		1
...
#endif
static int ts_charger_plug = 0;
static int ts_charger_type = 0;
...
struct mxt_data *mxt_test_data = NULL;
...
void trigger_baseline_state_machine(int plug_in, int type)
{
	if (plug_in == 0 || plug_in == 1) {
		if (plug_in == 0) {
			printk("[lge_touch] TA/USB NOT CONNECTED.\n");
			ts_charger_type = type;
			ts_charger_plug = plug_in;

			if (mxt_test_data && mxt_test_data->power_status) {
				mxt_change_cfg(mxt_test_data, mxt_test_data->T72_address, 0, t72_ctrl_cfg & MXT_DISABLE_MASK);
				mxt_change_cfg(mxt_test_data, mxt_test_data->T72_address, 0, t72_ctrl_cfg | MXT_ENABLE_MASK);
			}
		} else if (plug_in == 1) {
			printk("[lge_touch] %s CONNECTED.\n", type ? "TA" : "USB");
			ts_charger_type = type;
			ts_charger_plug = plug_in;
		}
	}
...
static void mxt_proc_t100_anti_message(struct mxt_data *data, u8 *message)
{
	...
	dev_dbg(dev, "anti touch area(%d), touch area(%d), internal area(%d)\n",
				data->anti->anti_area, data->anti->touch_area, data->anti->inter_area);

	if(data->ts_data.total_num >= 1 && data->anti->inter_area >= 0x15){
		dev_info(dev, "Run Anti touch Calibration(1).\n");
		mxt_t6_command(data, MXT_COMMAND_CALIBRATE, 1, false);
		mxt_reset_slots(data);
		data->anti->inter_area = 0;
		return;
	}

	if((t_ex_debug[TIME_CURR_TIME].tv_sec - t_ex_debug[TIME_START_TIME].tv_sec) <= 7){
		if(data->anti->anti_area >= 10){
			dev_info(dev, "Anti area %d Touch area %d\n", data->anti->anti_area, data->anti->touch_area);
			dev_info(dev, "Run Anti touch Calibration in 7sec\n");
			mxt_t6_command(data, MXT_COMMAND_CALIBRATE, 1, false);
			mxt_reset_slots(data);
		}
		data->anti->anti_area = 0;
		data->anti->touch_area = 0;
	}
}
...
static void mxt_proc_t100_message(struct mxt_data *data, u8 *message)
{
	...
	if(status & (1 << 2))
		dev_dbg(dev, "[%u] %c%c %s%s (%02X) x:%u y:%u amp:%u area:%02X vec:%02X\n",
			id,
...
char *vol_up[2] = { "TOUCH_VOLUME_UP=WAKEUP", NULL };
char *vol_down[2] = { "TOUCH_VOLUME_DOWN=WAKEUP", NULL };
static void mxt_proc_t35_messages(struct mxt_data *data, u8 *message)
{
	...
	if(data->mxt_character_enable && msg == 0x43){	/*C*/
		...
		kobject_uevent_env(&lge_touch_sys_device.kobj, KOBJ_CHANGE, vol_up);
	}else if(data->mxt_volume_enable && msg == 0x84){	/*Vol Down*/
		dev_info(dev, "Volume Down recognize.\n");
		kobject_uevent_env(&lge_touch_sys_device.kobj, KOBJ_CHANGE, vol_down);
	}else{
...
#endif
static void mxt_anti_report_disable(struct mxt_data *data);
static int mxt_proc_message(struct mxt_data *data, u8 *message)
{
	...
	} else if (report_id == data->T100_reportid_min && !data->suspended && !is_probing
				&& data->anti->anti_report){
		if((t_ex_debug[TIME_CURR_TIME].tv_sec - t_ex_debug[TIME_START_TIME].tv_sec) <= 15)
			mxt_proc_t100_anti_message(data, message);
		else
			mxt_anti_report_disable(data);
	} else if (report_id > data->T100_reportid_min
	    && report_id <= data->T100_reportid_max) {
	    if(!data->suspended && data->anti->autocal && (t_ex_debug[TIME_CURR_TIME].tv_sec - t_ex_debug[TIME_START_TIME].tv_sec) > 2){
			 mxt_write_reg(data->client, data->T8_address+4, 0);
			 data->anti->autocal = false;
			 dev_info(&data->client->dev, "Off Auto Cal.\n");
	    }
...
static void mxt_process_messages_t44(struct work_struct *work)
{
	...
	for (i = 0; i < data->pdata->numtouch; i++) {
		...
		if(!ts_charger_plug){
			if(data->ts_data.curr_data[i].status == FINGER_PRESSED){
				...
				if(data->ts_data.total_num >= 3){
					if(!data->anti->pen){
						data->anti->insensitive_th = true;
						dev_dbg(dev, "100 Flag\n");
					}else{
						data->anti->sensitive_th = true;
						dev_dbg(dev, "23 Flag PRESSED\n");
					}
				}
			}else if(data->ts_data.curr_data[i].status == FINGER_RELEASED){
				if(i == data->anti->pen_id){
					data->anti->pen = false;
					data->anti->pen_id = 255;
					dev_dbg(dev, "PEN RELEASED\n");
				}
				if(data->ts_data.total_num < 3){
					data->anti->sensitive_th = true;
					dev_dbg(dev, "23 Flag RELEASED\n");
				}
			}
		}
#endif
	...
	if(!ts_charger_plug){
		if(data->anti->curr_ths!= 100 && data->anti->insensitive_th){
			data->anti->insensitive_th = false;
			data->anti->curr_ths = 100;
			ret = mxt_write_reg(data->client, data->T100_address+32, 100);
			dev_info(dev, "Change Threshold to 100\n");
			if (ret)
				dev_err(dev, "Change Threshold error to 100\n");
		}else if(data->anti->curr_ths != 23 && data->anti->sensitive_th){
			data->anti->sensitive_th = false;
			data->anti->curr_ths = 23;
			ret = mxt_write_reg(data->client, data->T100_address+32, 23);
			dev_info(dev, "Change Threshold to 23\n");
			if (ret)
				dev_err(dev, "Change Threshold error to 23\n");
		}
		data->anti->insensitive_th = false;
		data->anti->sensitive_th = false;
	}
...
{
	//if(data->run_wakeup_enable || data->mxt_knock_on_enable || data->mxt_character_enable || data->mxt_volume_enable){
...
/*                                                   */
static void mxt_anti_report_enable(struct mxt_data *data)
{
	int ret = 0;

	dev_info(&data->client->dev, "%s\n", __func__);

	t100_scraux_cfg = t100_scraux_cfg | MXT_ANTI_ENABLE_MASK;
	dev_dbg(&data->client->dev, "anti touch T100 (addr %d |value %d)\n", data->T100_address+2, t100_scraux_cfg);
	ret = mxt_write_reg(data->client, data->T100_address+2, t100_scraux_cfg);
	if (ret) {
		dev_err(&data->client->dev, "antitouch enable fail.\n");
	}
	data->anti->anti_report = true;
}
...
static void mxt_anti_report_disable(struct mxt_data *data)
{
	...
	t100_scraux_cfg = t100_scraux_cfg & MXT_ANTI_DISABLE_MASK;
	dev_dbg(&data->client->dev, "anti touch T100 (addr %d |value %d)\n", data->T100_address+2, t100_scraux_cfg);
	ret = mxt_write_reg(data->client, data->T100_address+2, t100_scraux_cfg);
	if (ret) {
		dev_err(&data->client->dev, "antitouch disable fail.\n");
	}
	data->anti->anti_report = false;
}
...
#ifdef MXT_GESTURE_RECOGNIZE
	if(data->mxt_knock_on_enable || data->mxt_character_enable || data->mxt_volume_enable) {
		touch_disable_irq(data->irq);
		mxt_active_mode_start(data);
		/* Recalibrate since touch doesn't power off when lcd on */
		mxt_t6_command(data, MXT_COMMAND_CALIBRATE, 1, false);
	} else if (data->use_regulator && (data->power_status == MXT_POWER_OFF)) {
#else
	if (data->use_regulator) {
#endif
		mxt_regulator_enable(data);
				...
				struct mxt_config_info *info)
{
	...
	while(i < MXT_TMAX) {
		...
		if(info->config_t[i][0] == 47){
			...
			t100_scraux_cfg = info->config_t[i][3];
		}
#endif
...
static int touch_suspend(struct device *device)
{
	...
	printk("lge_touch Touch Suspend\n");
	return 0;
}
