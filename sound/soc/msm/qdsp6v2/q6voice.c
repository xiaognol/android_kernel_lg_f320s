					...
					struct voice_data **voice)
{
	...
	if (itr == NULL)
		return false;
	pr_info("%s : cur idx = %d session idx = %d",
...
uint32_t voc_get_session_id(char *name)
{
	...
	if (name != NULL) {
		...
		pr_info("%s: %s has session id 0x%x\n", __func__, name,
				session_id);
	}
...
static struct voice_data *voice_get_session(u32 session_id)
{
	...
	pr_info("%s:session_id 0x%x session handle 0x%x\n",
...
static int voice_apr_register(void)
{
	...
	if (common.apr_q6_mvm == NULL) {
		pr_info("%s: Start to register MVM callback\n", __func__);
	...
	if (common.apr_q6_cvs == NULL) {
		pr_info("%s: Start to register CVS callback\n", __func__);
	...
	if (common.apr_q6_cvp == NULL) {
		pr_info("%s: Start to register CVP callback\n", __func__);
...
static int voice_send_dual_control_cmd(struct voice_data *v)
{
	...
	pr_info("%s: Send Dual Control command to MVM\n", __func__);
	if (!is_voip_session(v->session_id)) {
		...
		mvm_voice_ctl_cmd.hdr.pkt_size = APR_PKT_SIZE(
						...
						APR_HDR_SIZE);
		pr_info("%s: send mvm Voice Ctl pkt size = %d\n",
	...
	if (!mvm_handle) {
		if (!is_voip_session(v->session_id)) {
			...
			mvm_session_cmd.hdr.pkt_size = APR_PKT_SIZE(
						...
						APR_HDR_SIZE);
			pr_info("%s: send mvm create session pkt size = %d\n",
...
static int voice_destroy_mvm_cvs_session(struct voice_data *v)
{
	...
	if (is_voip_session(v->session_id)) {
		pr_info("%s: MVM detach stream, VOC_STATE: %d\n", __func__,
				v->voc_state);
					...
					uint32_t module_id, int enable)
{
	...
	pr_info("voice_send_set_pp_enable_cmd, module_id=%d, enable=%d\n",
		module_id, enable);
...
static int voice_send_mvm_media_type_cmd(struct voice_data *v)
{
	...
	pr_info("%s: setting media_id as %x\n",
...
void voc_disable_dtmf_det_on_active_sessions(void)
{
	...
	for (i = 0; i < MAX_VOC_SESSIONS; i++) {
		...
		if ((v->dtmf_rx_detect_en) &&
			...
			pr_info("disable dtmf det on ses_id=%d\n",
				 v->session_id);
...
int voc_alloc_cal_shared_memory(void)
{
	...
	if (is_cal_memory_allocated()) {
		pr_info("%s: Calibration shared buffer already allocated",
			 __func__);
	} else {
...
static int voice_config_cvs_vocoder(struct voice_data *v)
{
	...
	case VSS_MEDIA_ID_4GV_NW_MODEM: {
		...
		pr_info("Setting EVRC min-max rate\n");
...
static int voice_send_disable_vocproc_cmd(struct voice_data *v)
{
	...
	pr_info("cvp_disable_cmd pkt size = %d, cvp_handle=%d\n",
				   ...
				   uint32_t *rx_topology_id)
{
	...
	if (v->lch_mode == VOICE_LCH_START) {
		pr_info("%s: Setting TX and RX topology to NONE for LCH\n",
			 __func__);
...
static int voice_send_set_device_cmd(struct voice_data *v)
{
	...
	pr_info(" send create cvp setdev, pkt size = %d\n",
	...
	pr_info("wait for cvp create session event\n");
	ret = wait_event_timeout(v->cvp_wait,
			(v->cvp_state == CMD_STATUS_SUCCESS),
			msecs_to_jiffies(TIMEOUT_MS));
	if (!ret) {
					 ...
					 uint32_t token)
{
	...
	pr_info("%s: next table desc: add: %lld, size: %d\n",
		 __func__, *((uint64_t *) memtable),
		 *(((uint32_t *) memtable) + 2));
	pr_info("%s: phy add of of mem being mapped 0x%x, size: %d\n",
...
static int voice_mem_map_cal_block(struct voice_data *v)
{
	...
	if (common.cal_mem_handle != 0) {
		pr_info("%s: Cal block already mem mapped\n", __func__);
...
static int voice_pause_voice_call(struct voice_data *v)
{
	...
	pr_info("%s: send mvm_pause_voice_cmd pkt size = %d\n",
...
static int voice_send_enable_vocproc_cmd(struct voice_data *v)
{
	...
	pr_info("cvp_enable_cmd pkt size = %d, cvp_handle=%d\n",
...
static int voice_destroy_vocproc(struct voice_data *v)
{
	...
	if (is_volte_session(v->session_id) &&
		...
		if (v->rec_info.rec_enable) {
			...
			pr_info("%s: switch recording, srvcc_rec_flag %d\n",
...
static int voice_send_cvs_packet_exchange_config_cmd(struct voice_data *v)
{
	...
	pr_info("%s: dec buf: add %p, size %d, enc buf: add %p, size %d\n",
		__func__,
		dec_buf,
...
static int voice_cvs_start_record(struct voice_data *v, uint32_t rec_mode)
{
	...
	if (!v->rec_info.recording) {
		...
		pr_info("%s: Start record already sent\n", __func__);
	}
...
static int voice_cvs_stop_record(struct voice_data *v)
{
	...
	if (v->rec_info.recording) {
		...
		pr_info("%s: Stop record already sent\n", __func__);
	}
	...
	while (voice_itr_get_next_session(&itr, &v)) {
		...
		if (set) {
			if ((v->rec_route_state.ul_flag != 0) &&
				...
				pr_info("%s: rec mode already set.\n",
					__func__);
...
static int voice_cvs_stop_playback(struct voice_data *v)
{
	...
	if (v->music_info.playing && ((!v->music_info.count) ||
		...
		pr_info("%s: Stop playback already sent\n", __func__);
	}
...
int voc_enable_cvp(uint32_t session_id)
{
	...
	if (v->voc_state == VOC_CHANGE) {
		...
		if (v->lch_mode == VOICE_LCH_START) {
			...
			pr_info("%s: Mute commands not sent for lch_mode=%d\n",
				 __func__, v->lch_mode);
		}
			...
			uint32_t ramp_duration)
{
	...
	pr_info("%s session id = %#x vol = %u", __func__, session_id,
		vol_step);
...
int voc_standby_voice_call(uint32_t session_id)
{
	...
	if (v == NULL) {
        ...
        pr_info("%s: voc state=%d", __func__, v->voc_state);
    }
	...
	if (v->voc_state == VOC_RUN) {
		...
		pr_info("send mvm_standby_voice_cmd pkt size = %d\n",
			 mvm_standby_voice_cmd.pkt_size);
...
int voc_set_lch(uint32_t session_id, enum voice_lch_mode lch_mode)
{
	...
	if (v->lch_mode == lch_mode) {
		pr_info("%s: Session %d already in LCH mode %d\n",
...
int voc_start_voice_call(uint32_t session_id)
{
	...
	if ((v->voc_state == VOC_INIT) ||
		...
		if (ret < 0) {
			pr_info("%s: Continue without calibration %d\n",
				 __func__, ret);
		}
...
static int32_t qdsp_mvm_callback(struct apr_client_data *data, void *priv)
{
	...
	pr_info("%s: Payload Length = %d, opcode=%x\n", __func__,
...
static int32_t qdsp_cvs_callback(struct apr_client_data *data, void *priv)
{
	...
	if (data->opcode == APR_BASIC_RSP_RESULT) {
		...
		pr_info("Encoder buffer is ready\n");
		...
		pr_info("Recd VSS_ISTREAM_EVT_SEND_ENC_BUFFER\n");
	} else if (data->opcode ==
		...
		if ((voc_pkt != NULL) &&
			...
			pr_info("RX_DTMF_DETECTED low_freq=%d high_freq=%d\n",
...
static int voice_alloc_oob_shared_mem(void)
{
	...
	pr_info("%s buf[0].data:[%p], buf[0].phys:[%p], &buf[0].phys:[%p],\n",
		 __func__,
		(void *)v->shmem_info.sh_buf.buf[0].data,
		(void *)v->shmem_info.sh_buf.buf[0].phys,
		(void *)&v->shmem_info.sh_buf.buf[0].phys);
	pr_info("%s: buf[1].data:[%p], buf[1].phys[%p], &buf[1].phys[%p]\n",
		__func__,
